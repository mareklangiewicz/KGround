...
data class UreCharClassUnion @NotPortableApi internal constructor(
  ...
  init {
    req(tokens.isNotEmpty()) { "No tokens in UreCharClassUnion." }
  }
...
data class UreCharClassRange @NotPortableApi constructor(
  val from: UreCharClass,
  val to: UreCharClass,
  val positive: Boolean = true,
) : UreCharClass {
  override fun toClosedIR(): IR = toIR()
...
data class UreCharClassIntersect @NotPortableApi @DelicateApi internal constructor(
  ...
  override fun toIR(): IR =
    tokens.joinToString("&&", if (positive) "[" else "[^", "]") { it.toIRInCharClass().str }.asIR
